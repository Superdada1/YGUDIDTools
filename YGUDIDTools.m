...
//  TestKeyChain
//
//  Created by Super-mac on 16/8/30.
//  Copyright © 2016年 tongbuwanglou. All rights reserved.
//

#import "YGUDIDTools.h"
...
static const char kKeyChainUDIDAccessGroup[] = "com.test.keyChain";

@implementation YGUDIDTools

+ (NSString*)UDID
{
    NSString *udid = [YGUDIDTools getUDIDFromKeyChain];
    if (!udid) {
        udid = [YGUDIDTools _UDID_iOS7];           //ios7以上普遍用这个方法

        [YGUDIDTools settUDIDToKeyChain:udid];
    }
...
+ (NSString*)_UDID_iOS6
{
    return [YGUDIDTools getMacAddress];
}
...
+ (BOOL)settUDIDToKeyChain:(NSString*)udid
{
    ...
    if ([YGUDIDTools getUDIDFromKeyChain]) {        // there is item in keychain
        [YGUDIDTools updateUDIDInKeyChain:udid];
        [dictForAdd release];
        return YES;
    }
